...
| Question data validation | Partial | Runtime validation and `npm run validate` check IDs, required fields, four options, answer index, explanations, and known canonical topic tags. README states actual seed size and validation reports canonical topic coverage. | Validation is not yet wired into CI/deploy and does not validate environment shape. | M1/M2: CI quality gate |
