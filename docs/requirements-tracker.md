...
| Guest persistence | Partial | Versioned local cache stores ordered IDs, score, mode, cap/shuffle settings, dataset version, stats, completed summaries, bookmarks, and a guest JSON export. Corrupt/stale data is normalized or invalidated. | No dedicated storage unit tests; account-level export/delete still belongs in future member accounts. | M1: stabilize guest quiz |
| Question data validation | Partial | A machine-readable per-record JSON Schema exists, runtime validation plus `npm run validate` check IDs, required fields, four options, answer index, explanations, unsupported fields, duplicate tags, and known canonical topic tags, and `npm run prod:test` adds a first production-test gate for validation, Svelte diagnostics, static build output, and obvious built-secret patterns. README states actual seed size and validation reports canonical topic coverage. | Needs CI/deploy wiring and provider-specific environment validation once backend hosting is selected. | M1/M2: CI quality gate |
| Accessibility/usability | Partial | Semantic buttons/headings/labels, visible focus styles, responsive layout, answer explanation announcements, keyboard shortcuts that ignore editable controls, no external icon dependency, disabled-start explanations, and reduced-motion CSS are present. | WCAG audit and screen-reader pass are still needed. | M1: accessibility pass |
